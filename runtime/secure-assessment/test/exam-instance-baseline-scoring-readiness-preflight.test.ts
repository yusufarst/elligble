...
describe('BU-069: Exam Instance Baseline Scoring Readiness Preflight', () => {
  ...
  test('1. invalid tenant UUID -> denied', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient(), 'invalid', VALID_EXAM, async () => true);
    assert.deepEqual(res, { type: 'denied' });
  });

  // 2. invalid exam UUID -> denied
  test('2. invalid exam UUID -> denied', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient(), VALID_TENANT, 'invalid', async () => true);
    assert.deepEqual(res, { type: 'denied' });
  });

  // 3. capability denied -> denied
  test('3. capability denied -> denied', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient(), VALID_TENANT, VALID_EXAM, async () => false);
    assert.deepEqual(res, { type: 'denied' });
  });

  // 4. capability unavailable -> unavailable
  test('4. capability unavailable -> unavailable', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient(), VALID_TENANT, VALID_EXAM, async () => { throw new Error('unavailable'); });
  ...
  test('6. missing Exam Instance -> denied', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient({ missingExam: true }), VALID_TENANT, VALID_EXAM, async () => true);
    assert.deepEqual(res, { type: 'denied' });
  });

  // 7. wrong tenant -> denied
  test('7. wrong tenant -> denied', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient({ wrongTenant: true }), '00000000-0000-0000-0000-000000000003', VALID_EXAM, async () => true);
    assert.deepEqual(res, { type: 'denied' });
  });

  // 8. non-SCHEDULED -> invalid_state
  test('8. non-SCHEDULED -> invalid_state', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient({ lifecycle_state: 'DRAFT' }), VALID_TENANT, VALID_EXAM, async () => true);
  ...
  test('9. zero snapshots -> question_snapshot_empty', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient({ question_snapshot_empty: true }), VALID_TENANT, VALID_EXAM, async () => true);
  ...
  test('10. one valid baseline snapshot -> baseline_scoring_ready', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient(), VALID_TENANT, VALID_EXAM, async () => true);
  ...
  test('13. multiple valid snapshots aggregate exact count', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient({ multipleValid: true }), VALID_TENANT, VALID_EXAM, async () => true);
  ...
  test('15. schema_version_invalid propagates through scoring_snapshot_invalid', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient({ invalidSchemaVersion: true }), VALID_TENANT, VALID_EXAM, async () => true);
  ...
  test('16. question_type_invalid propagates exactly', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient({ invalidQuestionType: true }), VALID_TENANT, VALID_EXAM, async () => true);
  ...
  test('17. correct_option_invalid propagates exactly', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient({ invalidCorrectOption: true }), VALID_TENANT, VALID_EXAM, async () => true);
  ...
  test('18. max_score_invalid propagates exactly', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient({ invalidMaxScore: true }), VALID_TENANT, VALID_EXAM, async () => true);
  ...
  test('19. non-scoring BU-066 blocker such as prompt_missing_or_empty is preserved exactly rather than reinterpreted', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient({ promptMissing: true }), VALID_TENANT, VALID_EXAM, async () => true);
  ...
  test('20. stable snapshot-id ordering determines first invalid snapshot', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient({ multipleInvalidOrder: true }), VALID_TENANT, VALID_EXAM, async () => true);
  ...
  test('21. no INSERT / UPDATE / DELETE', async () => {
    ...
    await checkExamInstanceBaselineScoringReadiness(createClient({}, checks), VALID_TENANT, VALID_EXAM, async () => true);
    assert.equal(checks.insertUpdateDelete, false);
  });

  // 22. database error -> unavailable
  test('22. database error -> unavailable', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient({ dbError: true }), VALID_TENANT, VALID_EXAM, async () => true);
    assert.deepEqual(res, { type: 'unavailable' });
  });
  ...
  test('23. capability evaluator called exactly once', async () => {
    let callCount = 0;
    await checkExamInstanceBaselineScoringReadiness(createClient(), VALID_TENANT, VALID_EXAM, async () => {
      callCount++;
      return true;
    });
  ...
  test('27. unknown extra frozen-content metadata that BU-066 permits does not become a new blocker', async () => {
    const res = await checkExamInstanceBaselineScoringReadiness(createClient({ extraMetadata: true }), VALID_TENANT, VALID_EXAM, async () => true);
    assert.equal(res.type, 'baseline_scoring_ready');
  });
});
