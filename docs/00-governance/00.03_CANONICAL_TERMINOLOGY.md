...
Teacher / TEACHER assignment → staff responsibility/context
Teaching Assignment      → time- and scope-bound relation describing which teacher teaches which subject to which academic scope in a defined period/context
Teacher                  ≠ Teaching Assignment
Proctor                  → Exam supervision assignment
Teacher                  ≠ Proctor

For an authorized teacher-managed assessment, the same Person may receive contextual exam supervision/monitoring capability for that specific Exam Instance by policy. This does NOT mean Teacher = Proctor globally.
